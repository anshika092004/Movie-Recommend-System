...
def fetch_person_details(id_):
    url = f"https://api.themoviedb.org/3/person/{id_}?api_key=544db60ca312982c71ffdb5fb1fa2736"
    ...
    try:
        ...
        print("⚠️ Network error: Could not connect to TMDB API")
        return "https://via.placeholder.com/220x330.png?text=No+Image", "⚠️ Network error"
    except requests.exceptions.Timeout:
        print("⚠️ Request timed out")
        return "https://via.placeholder.com/220x330.png?text=Timeout", "⚠️ Request timed out"
    except Exception as e:
        print("⚠️ Error while fetching details:", e)
        return "https://via.placeholder.com/220x330.png?text=Error", f"⚠️ Error: {e}"

    # Safely extract fields
    try:
        profile_path = data.get("profile_path")
        url = "https://image.tmdb.org/t/p/w220_and_h330_face" + profile_path if profile_path else "https://via.placeholder.com/220x330.png?text=No+Image"
        biography = data.get("biography") or "No biography available"
    except Exception:
        url = "https://via.placeholder.com/220x330.png?text=No+Image"
